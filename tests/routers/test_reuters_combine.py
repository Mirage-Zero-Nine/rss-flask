...
def test_title_cache_key_reuses_existing_same_title_within_window():
    router = build_router()
    existing = [
        {
            "title": "  Same   Story ",
    ...
    # 22 hours apart: outside the old 3h window, well within the new 24h window.
...
def test_title_cache_key_uses_different_date_outside_window():
    router = build_router()
    existing = [
        {
            "title": "Same Story",
    ...
    # 25 hours apart: just outside the 24h window; falls through to a different
    # dated key.
    cache_key = router._build_title_cache_key(
        "Same Story",
...
def test_title_cache_key_disambiguates_when_timestamps_missing():
    """Without parseable timestamps, the candidate falls through to a title-only
    key. If that collides with an existing row that also lacks a parseable time,
    the collision handler must disambiguate using the stable source identifier.
    """
    router = build_router()
    existing_key = router._build_title_cache_key("Same Story", None, "old", [])
    existing = [
        {
            "title": "Same Story",
            "created_time": None,
...
def test_reuters_content_failure_leaves_existing_yahoo_content_untouched(monkeypatch):
    ...
    # Reuters returned the row in the list but its content fetch failed.
    # Per spec (AGENTS.md "Reuters combined refresh path"), replacement is
    # gated on a successful Reuters content fetch, so both the Yahoo article
    # payload AND the Yahoo metadata row must remain untouched.
    assert refreshed is True
    assert feed_items[final_key]["description"] == "<p>Yahoo body</p>"
    assert metadata_lists[article_list_key][0]["link"] == "https://www.yahoo.com/news/articles/shared.html"
    assert metadata_lists[article_list_key][0]["guid"] == "https://www.yahoo.com/news/articles/shared.html"
