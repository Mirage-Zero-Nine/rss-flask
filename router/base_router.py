...
class BaseRouter:
    ...
    def refresh_cache(self, parameter=None, link_filter=None, title_filter=None, force=False):
        ...
        if not article_metadata_list:
            logging.warning(
                "Router %s refresh returned 0 articles for parameter=%s; publishing empty metadata list",
                self.router_path,
                parameter,
            )
            ...
            logging.info(
                "Router %s recorded empty refresh at: %s",
                self.router_path,
                last_build_time.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3],
            )
            return False
        ...
        for article_metadata in article_metadata_list:
            ...
            if cached_entry is None:
                logging.warning(
                    "Router %s article content unavailable in cache for link=%s; metadata will still be published",
                    self.router_path,
        ...
        logging.info(
            ...
            self.router_path,
            len(article_metadata_list),
            cached_article_count,
            len(cached_metadata) if cached_metadata else 0,
            parameter,
        )
