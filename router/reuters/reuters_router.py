...
class ReutersRouter(BaseRouter):
    ...
    _title_match_threshold = timedelta(hours=24)
    ...
    def _acquire_metadata_lock(self, category: str, source: str, incoming_count: int) -> str | None:
        ...
        if token is not None:
            logging.debug(
                "Reuters combined metadata lock acquired lock=%s category=%s source=%s incoming_count=%d",
                lock_name, category, source, incoming_count,
            )
            return token

        logging.info(
            "Reuters combined metadata lock contended on first attempt lock=%s category=%s source=%s; retrying after %.1fs",
            lock_name, category, source, self._cache_lock_retry_seconds,
        )
        ...
        if token is None:
            ...
            logging.info(
                "Reuters combined metadata lock acquired on retry lock=%s category=%s source=%s",
                lock_name, category, source,
            )
        return token

    def _release_metadata_lock(self, category: str, token: str) -> None:
        lock_name = self._lock_name_for_category(category)
        logging.debug("Reuters combined metadata lock released lock=%s category=%s", lock_name, category)
        release_cache_lock(lock_name, token)
    ...
    def refresh_cache(
        ...
        category = parameter["category"]
        logging.info("Reuters combined refresh start category=%s force=%s", category, force)
        ...
        logging.info(
            "Reuters combined refresh fetched article list category=%s articles=%d",
            category, len(incoming_metadata),
        )
        ...
        content_failures = len(incoming_metadata) - len(fetched_entries)
        logging.info(
            "Reuters combined refresh content build category=%s succeeded=%d/%d failed=%d",
            category, len(fetched_entries), len(incoming_metadata), content_failures,
        )

        token = self._acquire_metadata_lock(category, "reuters", len(incoming_metadata))
        if token is None:
            logging.warning(
                "Reuters combined refresh aborted category=%s: metadata lock unavailable; %d fetched payloads not persisted",
                category, len(fetched_entries),
            )
            return False
        ...
        try:
            ...
            articles_written = 0
            articles_replaced_existing = 0
            key_changes = 0
            for metadata in incoming_metadata:
                provisional_key = metadata.cache_key
                entry = fetched_entries.get(provisional_key)
                if not (entry and entry.description):
                    # Reuters did not return article content for this row this
                    # cycle. Drop the Reuters metadata row so any existing
                    # Yahoo (or earlier Reuters) metadata + payload for the
                    # same story remains untouched. Replacement of Yahoo with
                    # Reuters is gated on a successful Reuters content fetch,
                    # not on Reuters list-fetch success alone (see AGENTS.md
                    # "Reuters combined refresh path").
                    continue
                ...
                if metadata.cache_key != provisional_key:
                    key_changes += 1
                    logging.debug(
                        "Reuters combined key resolved category=%s provisional=%s final=%s title=%s",
                        category, provisional_key, metadata.cache_key, metadata.title[:60],
                    )
                # Read the shared cache slot before overwriting so we can
                # surface override events for debugging (e.g., Reuters
                # upgrading a Yahoo-filled slot).
                existing_payload = read_feed_item_from_cache(metadata.cache_key)
                has_existing_content = bool(existing_payload and existing_payload.get("description"))
                entry.persist_to_cache(self.router_path, cache_key_override=metadata.cache_key)
                articles_written += 1
                if has_existing_content:
                    articles_replaced_existing += 1
                    logging.info(
                        "Reuters combined override category=%s key=%s title=%s previous_link=%s new_link=%s",
                        category,
                        metadata.cache_key,
                        metadata.title[:80],
                        existing_payload.get("link") if existing_payload else None,
                        metadata.link,
                    )
                else:
                    logging.debug(
                        "Reuters combined wrote fresh payload category=%s key=%s title=%s link=%s",
                        category,
                        metadata.cache_key,
                        metadata.title[:80],
                        metadata.link,
            ...
            logging.info(
                "Reuters combined refresh complete category=%s metadata_published=%d merged_total=%d articles_written=%d articles_replaced_existing=%d articles_fetched=%d key_changes=%d previous_metadata=%d",
                category,
                len(final_metadata),
                len(merged_metadata),
                articles_written,
                articles_replaced_existing,
                len(fetched_entries),
                key_changes,
                len(latest_metadata),
            )
    ...
    def _get_yahoo_article_content(self, article_metadata: Metadata, entry: FeedItem) -> None:
        ...
        try:
            ...
            logging.warning(
                "Reuters Yahoo-source redirect loop fetching article link=%s: %s",
                article_metadata.link, exc,
            )
            entry.description = ""
            return
        except Exception:
    ...
    def refresh_from_yahoo(self) -> bool:
        logging.info("Reuters Yahoo-source refresh start categories=%s", sorted(self.combined_categories))
        soup = self._fetch_yahoo_page(self.yahoo_reuters_page)
        if soup is None:
            logging.warning("Reuters Yahoo-source skipping refresh: page fetch returned None")
            return False
        ...
        for category in self.combined_categories:
            ...
            if not incoming_metadata:
                logging.info(
                    "Reuters Yahoo-source category=%s parsed 0 articles, skipping (existing_metadata=%d)",
                    category, len(existing_metadata),
                )
                category_outcomes[category] = "no_articles"
                continue

            logging.info(
                "Reuters Yahoo-source category=%s parsed=%d existing_metadata=%d",
                category, len(incoming_metadata), len(existing_metadata),
            )
            ...
            content_failures = len(incoming_metadata) - len(fetched_entries)
            logging.info(
                "Reuters Yahoo-source content build category=%s succeeded=%d/%d failed=%d",
                category, len(fetched_entries), len(incoming_metadata), content_failures,
            )
            ...
            if token is None:
                logging.warning(
                    "Reuters Yahoo-source category=%s metadata lock unavailable; %d fetched payloads not persisted",
                    category, len(fetched_entries),
                )
                category_outcomes[category] = "lock_unavailable"
                continue
            ...
            try:
                ...
                for metadata in incoming_metadata:
                    ...
                    if metadata.cache_key != provisional_key:
                        key_changes += 1
                        logging.debug(
                            "Reuters Yahoo-source key resolved category=%s provisional=%s final=%s title=%s",
                            category, provisional_key, metadata.cache_key, metadata.title[:60],
                        )
                    ...
                    if cached_payload and cached_payload.get("description"):
                        skipped_existing += 1
                        logging.debug(
                            "Reuters Yahoo-source skip-write (slot has content) category=%s key=%s title=%s",
                            category, metadata.cache_key, metadata.title[:60],
                        )
                    else:
                        entry = fetched_entries.get(provisional_key)
                        if entry and entry.description:
                            ...
                            articles_written += 1
                            logging.info(
                                "Reuters Yahoo-source filled empty slot category=%s key=%s title=%s link=%s",
                                category,
                                metadata.cache_key,
                                metadata.title[:80],
                                metadata.link,
                ...
                replace_metadata_list(article_list_key, merged_metadata)
                logging.info(
                    "Reuters Yahoo-source category=%s wrote=%d skipped_existing=%d skipped_no_payload=%d "
                    "key_changes=%d metadata_total=%d merged_total=%d previous_metadata=%d",
                    category,
                    articles_written,
                    skipped_existing,
                    skipped_no_payload,
                ...
                category_outcomes[category] = (
                    f"wrote={articles_written},skipped_existing={skipped_existing},skipped_no_payload={skipped_no_payload}"
                )
            finally:
                self._release_metadata_lock(category, token)

        logging.info(
            "Reuters Yahoo-source refresh complete processed_any=%s outcomes=%s",
            processed_any, category_outcomes,
        )
