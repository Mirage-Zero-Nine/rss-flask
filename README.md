...
See `requirements.txt`. It is generated by `pip3 freeze > requirements.txt`

### Redis cache

- The service now persists router metadata and entries in Redis instead of local JSON files. Install a compatible Redis server (default `redis://localhost:6379/0`) and set `RSS_REDIS_URL` if you need a different connection string before starting the container.
- You can also record the URL in `config.yml` via the new `rss_redis_url` field; the app reads that value when `RSS_REDIS_URL` is unset.
